-- TO EDIT TABLES https://www.w3schools.com/sql/sql_ref_alter_table.asp
...
-- test that data importing successful
...
-- filter data for retirement eligibility
...
-- Create a new table in order to make a CSV file
SELECT first_name, last_name
...
-- Making table of current employees retiring soon
SELECT ri.emp_no,
    ri.first_name,
    ri.last_name,
	de.to_date
...
-- Create new table on employee information
SELECT * FROM salaries
...
-- Create new table on manager information
...
-- Create table of department retirees
SELECT ce.emp_no,
ce.first_name,
ce.last_name,
d.dept_name
...
-- SKILL DRILL 7.3.6 -- select employee info and department name for one specific dept.
...
-- SKILL DRILL -- add in another department to the previous search using IN condition
SELECT ri.emp_no, ri.first_name, ri.last_name, d.dept_name
FROM retirement_info AS ri
LEFT JOIN dept_emp AS de
ON ri.emp_no = de.emp_no
